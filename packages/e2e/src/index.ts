import { createJestRunner } from '@jestify/jest-wrapper';
import { afterEach, beforeEach, createSuite, TestWorker } from 'describers';
...
async function runTasksConcurrently(
  tasks: Array<(worker: TestWorker) => Promise<void>>,
  maxWorkers: number,
) {
...
function assertBrowserName(
  browserName: string,
): asserts browserName is 'webkit' | 'chromium' | 'firefox' {
  if (
    browserName !== 'firefox' &&
...
const Runner = createJestRunner(
  async (options, jestRequireAndTransform) => {
  ...
  async (tests, options, onStart, onResult) => {
    ...
    await Promise.all(
      [...browserPromiseForName.values()].map(async (browserPromise) =>
...
async function listTestsInternal(
  options: { path: string; rootDir: string },
  requireCallback: () => void,
) {
  installGlobals();
  const suite = createSuite(async () => {
    beforeEach(async (state) => {
      ...
      state.context = await (
        await ensureBrowserForName(state.browserName)
      ).newContext();
      ...
      return config.browsers.map((browserName) => {
        const titles =
          config.browsers.length === 1
            ? describersTest.ancestorTitles()
            : [browserName, ...describersTest.ancestorTitles()];
        return {
          describersTest,
          browserName,
