...
export async function installSuperpowers(page: Page) {
  ...
  const api: { [key: string]: any } = {
    ...
    network: {
      route: async (
        urlOrRegexOrPredicate: string,
        handler: { isFunction: true; index: number },
      ) => {
        ...
        await page.route(urlOrRegexOrPredicate, async (route, request) => {
          ...
          await page
            .evaluate(
              async ({ handlerIndex, requestObj, routeId }) => {
                ...
                const request: RequestApi = {
                  ...
                  failure() {
                    return requestObj.failure;
                  },
                  postData() {
                    return requestObj.postData;
                  },
                  isNavigationRequest() {
                    return requestObj.isNavigationRequest;
                  },
                };
                const route: RouteApi = {
                  fulfill: (obj) =>
                    superpowers('network', '_fulfill', routeId, obj),
                  abort: (obj) =>
                    superpowers('network', '_abort', routeId, obj),
                  continue: (obj) =>
                    superpowers('network', '_continue', routeId, obj),
                  request: () => request,
                };
      ...
      async _continue(
        routeId: number,
        params: Parameters<Route['continue']>[0],
      ) {
        const route = routes.get(routeId);
  ...
  await page.exposeFunction(
    '__superpowers__',
    (category: string, method: string, ...args: any) => {
  ...
  for (const [category, value] of Object.entries(api)) {
    ...
    for (const method of Object.keys(value)) {
      ...
      script.push(
        `window.${category}.${method} = (...args) => __superpowers__('${category}', '${method}', ...args.map(serialize));`,
      );
    }
  }
