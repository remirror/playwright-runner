...
export function createJestRunner<TestDefinition extends { titles: string[] }>(
  testsAtPath: (
    ...
    jestRequireAndTransform: () => void,
  ) => Promise<TestDefinition[]>,
  runTests: (
    tests: TestDefinition[],
    options: TestRunOptions,
    onStart: (test: TestDefinition) => Promise<void>,
    onResult: (test: TestDefinition, result: TestResult) => Promise<void>,
  ) => Promise<void>,
) {
  return class {
    runTests: (
      testSuites: JestSuite[],
      watcher: TestWatcher,
      onStart: OnTestStart,
    ...
    constructor(
      ...
      this.runTests = async (
        ...
        const filterRegex = globalConfig.testNamePattern
              ...
              await testsAtPath(
                ...
                () => {
                  const transformer = new ScriptTransformer(
                    testSuite.context.config,
                  );
                  transformer.requireAndTranspileModule(testSuite.path);
                },
              )
            ).filter((test) =>
              filterRegex ? filterRegex.test(test.titles.join(' ')) : true,
            );
          ...
          async (test, result) => {
            ...
            const assertionResults = tests.map((test) => {
              ...
              const jestResult: JestTestResult.AssertionResult = {
                ancestorTitles: test.titles.slice(0, -1),
                failureMessages: [],
              ...
              if (result.status === 'fail') {
                ...
                jestResult.failureMessages.push(
...
function makeSuiteResult(
  assertionResults: JestTestResult.AssertionResult[],
  testPath: string,
): SuiteResult {
